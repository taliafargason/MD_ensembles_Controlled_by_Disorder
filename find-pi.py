...
def find_ring_center(ring):
    ...
    # Initialize variables to hold center and face normal
    center = None
    face_norm = None

    # Check if 'CG' and 'CD1' keys exist in the ring dictionary
    if 'CG' in ring and 'CD1' in ring:
        # Calculate center using all six carbons
        center = (ring['CG'] + ring['CD1'] + ring['CD2'] + ring['CE1'] + ring['CE2'] + ring['CZ']) / 6
        # Calculate face of the ring
    ...
    elif 'CD2' in ring and 'CE2' in ring:
        # Calculate center using 'CD2' and 'CE2'
        center = (ring['CD2'] + ring['CE2']) / 2
        # Calculate face of the ring
        vec1 = np.array([center, ring['CE2']])
        vec2 = np.array([center, ring['CD2']])
        v1 = np.subtract(vec1[1, ...], vec1[0, ...])
        v2 = np.subtract(vec2[1, ...], vec2[0, ...])
        face_norm = np.cross(v1, v2)
    else:
        # Handle cases where neither 'CD1' nor 'CD2' is present
        print("Neither 'CD1' nor 'CD2' found in the ring.")
